  ...
  class NoMethodError
    ...
    def to_s
      @to_s ||= begin
        display = ""
        display << separator
        display << class_and_message(@parsed) << "\n"
        display << separator
    ...
    def parse_exception(e)
      message_to_array = e.message.split(/\W+/)
      second_line_of_backtrace = e.backtrace[0]
      {
        exception_class: e.class,
        containing_object_type: message_to_array[5],
        containing_object_name: message_to_array[4],
        calling_method_name: second_line_of_backtrace.split(/\W+/)[-1],
        undefined_method_name: message_to_array[2]
      }
    end

    def class_and_message(parsed)
      white   = "\e[38;5;255m"
      bri_red = "\e[38;5;196m"
      dim_red = "\e[38;5;124m"
      none    = "\e[39m"
      "#{white}#{parsed.fetch :exception_class} | "\
      "#{bri_red}An undefined method called #{parsed.fetch :undefined_method_name} "\
      "#{bri_red}was called in #{parsed.fetch :containing_object_type} #{parsed.fetch :containing_object_name}"\
      "#{none}"
    end
  end
