...
at_exit do
  ...
  when ArgumentError # really, this only works for wrong number of arguments
    $stderr.puts WhatWeveGotHereIsAnErrorToCommunicate::ArgumentError.new(exception)
    exit! 1 # there has got to be a better way to clear an exception, this could break other at_exit hooks -.^
  when NoMethodError
    $stderr.puts WhatWeveGotHereIsAnErrorToCommunicate::NoMethodError.new(exception)
    exit! 1
  end
end
